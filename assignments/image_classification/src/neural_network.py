...
def save_classification_report(classification_report, output_dir, file_name="neural_classification_report.txt"):
...
def plot_loss_curve(classifier, output_dir, file_name="neural_loss_curve", file_format = 'png', save_file=True):
    ...
    if save_file:
        os.makedirs(output_dir, exist_ok=True)
        plt.savefig(os.path.join(output_dir, file_name + file_format))
...
def main():
    ...
    output_dir_path = os.path.join(os.path.dirname(__file__), "..", "out", "neural_network")
