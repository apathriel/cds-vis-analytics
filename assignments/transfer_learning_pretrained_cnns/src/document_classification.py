...
def load_image_training_data(data_path):
    try:
        ...
        loaded_images = [load_img(img, color_mode='rgb', target_size=(224, 224)) for img in tqdm(image_files, desc="Loading images")]
        # convert images to numpy arrays and preprocess for VGG16 model
        preprocessed_images = np.array([preprocess_input(img_to_array(img)) for img in tqdm(loaded_images, desc="Preprocessing images")])
        logger.info(f"Loaded {len(preprocessed_images)} images and {len(labels)} labels.")
...
def instantiate_VGG16_model(model_include_top=False, model_pooling="avg", model_input_shape=(224, 224, 3), disable_conv_training=True): 
    model = VGG16(include_top=model_include_top, pooling=model_pooling, input_shape=model_input_shape)
...
def define_classification_layers(model, batch_normalization_layer=True, dropout_layer=False, print_summary=False):
...
def instantiate_optimizer(optimizer_type='Adam', init_learn_rate=0.01, decay_steps=100000, decay_rate=0.9):
...
def save_trained_model(model, output_dir, model_format="keras", file_name="VGG16_tobacco_model"):
...
def plot_history(H, num_of_epochs: int, save_plot: bool =False, output_dir: str ='out', plot_name: str ="VGG16_tobacco_plot", plot_format: str ="png"):
    valid_output_formats = ['eps', 'jpeg', 'jpg', 'pdf', 'pgf', 'png', 'ps', 'raw', 'rgba', 'svg', 'svgz', 'tif', 'tiff']
    ...
    plt.plot(np.arange(0, num_of_epochs), H.history["val_loss"], label="val_loss", linestyle=":")
    ...
    plt.plot(np.arange(0, num_of_epochs), H.history["val_accuracy"], label="val_acc", linestyle=":")
...
def save_classification_report(classification_report, output_dir, log_output=True, file_name="VGG16_tobacco_report.txt"):
    if log_output:
...
if __name__ == '__main__':
    path_to_input_folder = os.path.join(os.path.dirname(os.path.abspath(__file__)),'..', 'in')
    ...
    VGG16_report = classification_report(y_test.argmax(axis=1),
                                predictions.argmax(axis=1),
                                target_names=labels,
                                zero_division=1)

    save_classification_report(VGG16_report, "out", file_name="VGG16_tobacco_report.txt")
    plot_history(H, 10, save_plot=True, output_dir="out", plot_name="VGG16_tobacco_plot", plot_format="pdf")